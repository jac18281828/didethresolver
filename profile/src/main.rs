use ethenv::{init, environment, printenv};
...
async fn main() {
    ...
    let contract = DidEthRegistry::new(env.rpc_url, env.private_key).await.unwrap();
    ...
    for (key, value) in env.attribute.iter() {
        println!("set_attribute - key: {}, value: {}", key, value);
        let tx_receipt = contract.set_attribute(key.clone(), value.clone()).await.unwrap();
